        ...
        {show && (
          <motion.div
            ...
            style={{
              ...
              background: "var(--surface)", border: "1px solid rgba(0,0,255,0.4)",
...
export function ProofCard({ pnl, proof, wallet, onDownload, isCapturing = false }: ProofCardProps) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: "radial-gradient(ellipse at 70% 0%, rgba(0,0,255,0.2) 0%, transparent 55%), var(--bg-deep)",
      ...
      <div style={{ position: "relative", zIndex: 3 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "24px" }}>
          <div style={{ display: "flex", background: "rgba(0,0,255,0.05)", border: "1px solid rgba(0,0,255,0.2)" }}>
            <button onClick={() => setMode('private')} style={{ background: mode === 'private' ? "rgba(0,0,255,0.2)" : "transparent", padding: "6px 14px", border: "none", color: mode === 'private' ? "var(--foreground)" : "var(--text-muted)", fontSize: "10px", cursor: "pointer", fontFamily: "'Share Tech Mono', monospace", transition: "all 0.2s" }}>PRIVATE</button>
            <button onClick={() => setMode('public')} style={{ background: mode === 'public' ? "rgba(0,0,255,0.2)" : "transparent", padding: "6px 14px", border: "none", color: mode === 'public' ? "var(--foreground)" : "var(--text-muted)", fontSize: "10px", cursor: "pointer", fontFamily: "'Share Tech Mono', monospace", transition: "all 0.2s" }}>PUBLIC PROOF</button>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "32px", position: "relative", minHeight: isCapturing && mode === 'private' ? "160px" : undefined }}>
          ...
          <div className="mono" style={{ fontSize: "10px", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "3px", marginBottom: "12px" }}>
            Verified Net Yield
          </div>
          <div
            className="bc"
          ...
          {isCapturing && mode === 'private' && (
            <div style={{
              ...
              background: "var(--bg-deep)", opacity: 0.95,
          ...
          >
            {[
              ...
              <div key={b.text} className="mono" style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "9px", color: "var(--text-muted)", letterSpacing: "0.5px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "20px" }}>
          <div>
            <div className="mono" style={{ fontSize: "9px", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "6px" }}>Prover Identity</div>
            <div className="mono" style={{ fontSize: "13px", color: "var(--text-dim)" }}>{shortenWallet(wallet)}</div>
          </div>
        ...
        <div style={{ background: "rgba(0,0,255,0.05)", border: "1px solid rgba(0,0,255,0.2)", padding: "16px", marginTop: "24px", display: "flex", flexDirection: "column", gap: "10px" }}>
           ...
           <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
             <div className="mono" style={{ fontSize: "10px", color: "var(--text-muted)", display: "flex", justifyContent: "space-between" }}>
               <span>Proof ID:</span>
               <span style={{ color: "var(--text-dim)" }}>{mode === 'public' ? `0x${proof.slice(0, 4).toUpperCase()}...${proof.slice(-4).toUpperCase()}` : "████████████████████"}</span>
             </div>
             <div className="mono" style={{ fontSize: "10px", color: "var(--text-muted)", display: "flex", justifyContent: "space-between" }}>
               <span>Formula:</span>
           ...
           <div className="mono" style={{ fontSize: "10px", color: "#4ade80", borderTop: "1px solid rgba(0,0,255,0.1)", paddingTop: "10px", marginTop: "4px", display: "flex", flexDirection: "column", gap: "10px" }}>
             <p style={{ color: "var(--text-muted)", fontSize: "9px", lineHeight: 1.4 }}>Proof is generated via deterministic hashing; execution model aligns with confidential computing (TEE).</p>
             <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
               ...
               <button
                ...
                style={{
                  ...
                  color: verifyState === 'verified' ? "#4ade80" : "var(--foreground)",
                  padding: "4px 8px",
