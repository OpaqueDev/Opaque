...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-deep)", color: "var(--foreground)", position: "relative" }}>

      {/* Mobile Overlay */}
      <div className={`mobile-overlay ${mobileOpen ? 'open' : ''}`} onClick={() => setMobileOpen(false)}></div>

      {/* Sidebar */}
      <aside className={`dash-sidebar ${mobileOpen ? 'open' : ''}`} style={{ width: "260px", borderRight: "2px solid var(--border)", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "32px 24px", background: "var(--sidebar-bg)" }}>
        <div>
           <Link href="/" className="bc" style={{ fontSize: "32px", fontWeight: 900, color: "var(--foreground)", textDecoration: "none", letterSpacing: "-1px" }}>OPAQUE_</Link>
        ...
        <div>
           {mounted && isConnected ? (
             <button className="btn-outline mono" style={{ width: "100%", padding: "12px", fontSize: "11px", borderColor: "var(--border)", color: "var(--text-dim)", background: "transparent" }} onClick={() => disconnect()}>
           ...
           <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "2px solid var(--border)" }}>
             <Link href="/" className="mono" style={{ color: "var(--text-muted)", fontSize: "11px", textDecoration: "none", display: "flex", alignItems: "center", gap: "8px", transition: "color 0.2s" }} onMouseOver={e => (e.currentTarget.style.color = "var(--foreground)")} onMouseOut={e => (e.currentTarget.style.color = "var(--text-muted)")}>
               ← RETURN TO HOME
             </Link>
           </div>
        </div>
      ...
      <main className="dash-main" style={{ flex: 1, height: "100vh", overflowY: "auto" }}>
        ...
        <header className="dash-header-wrap" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "48px", borderBottom: "2px solid var(--border)", paddingBottom: "24px" }}>
           <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
             ...
             <div className="mono" style={{ color: "var(--text-muted)", fontSize: "12px", marginTop: "8px" }}>iExec Nox TEE Enclave</div>
            </div>
           </div>
